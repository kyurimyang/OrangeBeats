// URL input screen — paste link, choose title strategy
// Globals: URLScreen

function URLScreen({ onNavigate, initialUrl }) {
  const [url, setUrl] = React.useState(initialUrl || '');
  const [titleMode, setTitleMode] = React.useState('youtube'); // 'youtube' | 'custom'
  ...
  const handleAnalyze = () => {
    ...
    onNavigate('loading', {
      url,
      title: titleMode === 'custom' ? customTitle : null,
    });
  };

  return (
    <div className="ob-fade min-h-screen pt-[68px] flex flex-col">
      {/* glow */}
      <div className="pointer-events-none fixed inset-0 -z-10">
        <div className="absolute top-1/3 left-1/2 -translate-x-1/2 w-[900px] h-[700px] rounded-full"
             style={{ background:'radial-gradient(closest-side, rgba(253,109,17,0.25), transparent 70%)' }}/>
        <div className="absolute bottom-1/4 right-10 w-[420px] h-[420px] rounded-full"
        ...
        <div className="mx-auto w-full max-w-[1080px] px-8 -mt-10">
          <div className="text-center">
            ...
            <h1 className="mt-6 font-display text-[56px] md:text-[64px] tracking-[-0.03em] leading-[1.02] font-medium">
              <span className="ob-grad-text">Playlist를 생성할</span><br/>
              Youtube URL을 입력해주세요.
            </h1>
          ...
          <div className="mt-12 max-w-[800px] mx-auto">
            <div className={`ob-input-shell ${error?'is-error':''}`}>
              <div className="ob-input-inner h-[78px] flex items-center pl-7 pr-2 gap-3">
                ...
                />
                {url && (
                  <button onClick={()=>setUrl('')} className="w-8 h-8 rounded-full hover:bg-white/10 flex items-center justify-center text-white/40 hover:text-white">
                    <IconClose size={14}/>
                  </button>
                )}
                <button
            ...
            <div className="mt-3 min-h-[24px] text-center">
              {error ? (
                ...
                <span className="text-[13px] text-white/40">예시 · youtube.com/playlist?list=… 또는 youtu.be/…</span>
              )}
            </div>
          </div>
...
function TitleOption({ active, onClick, label, hint, example, custom, customValue, onCustomChange }) {
  return (
    <button
      onClick={onClick}
      className={`relative text-left p-5 rounded-2xl transition ${active ? 'ob-glass' : 'ob-glass-soft hover:bg-white/[0.05]'}`}
    >
...
Object.assign(window, { URLScreen });
