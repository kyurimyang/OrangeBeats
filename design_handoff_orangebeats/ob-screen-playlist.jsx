// Final playlist screen — created, ready to open, with rating
// Globals: PlaylistScreen

function PlaylistScreen({ onNavigate }) {
  ...
  return (
    <div className="ob-fade min-h-screen pt-[68px] pb-24">
      ...
      <div className="pointer-events-none fixed inset-x-0 top-0 -z-10 h-[600px]">
        <div className="absolute top-32 left-1/2 -translate-x-1/2 w-[1100px] h-[700px] rounded-full"
      ...
      <div className="mx-auto max-w-[1280px] px-8">
        ...
        <div className="text-center mt-12">
          ...
          <h1 className="mt-7 font-display text-[64px] tracking-[-0.03em] leading-[0.98] font-medium">
            <span className="ob-grad-text">Playlist를</span><br/>
            <span>성공적으로 만들었어요!</span>
          </h1>
          <p className="mt-4 text-white/55 text-[15px]">
            8곡 중 8곡 모두 매칭됐어요. 지금 바로 스트리밍에서 열어보세요.
          </p>
        </div>

        {/* playlist hero card */}
        <div className="mt-12 ob-glass rounded-[28px] p-8 relative overflow-hidden">
          <div className="absolute -right-32 -top-32 w-[500px] h-[500px] rounded-full"
          ...
          <div className="relative grid grid-cols-12 gap-8 items-end">
            ...
            <div className="col-span-3">
              <div className="aspect-square rounded-2xl overflow-hidden relative"
                ...
                <div className="absolute inset-0 flex flex-col p-6 justify-between">
                  ...
                  <div>
                    <div className="text-[28px] font-display font-medium leading-[0.95] text-white">늦여름 드라이브</div>
            ...
            <div className="col-span-9">
              <div className="text-[11px] font-mono uppercase tracking-[0.18em] text-white/40">Playlist · created</div>
              <h2 className="mt-2 font-display text-[44px] tracking-[-0.03em] leading-[1.05] font-medium">
                {DEMO_VIDEO.title}
              </h2>
              <div className="mt-4 flex items-center gap-2 text-[13px] text-white/55">
                ...
                <span>약 36분</span>
                <span className="text-white/30">·</span>
                <span className="text-mint">평균 일치율 89%</span>
              </div>

              <div className="mt-7 flex items-center gap-3">
                <button className="ob-btn-primary h-12 px-6 rounded-full text-[14px] font-semibold inline-flex items-center gap-2">
                ...
                <button className="ob-btn-ghost h-12 w-12 rounded-full flex items-center justify-center text-white/85">
                  <IconExternal size={14}/>
                </button>
              </div>
            </div>
        ...
        <div className="mt-10 grid grid-cols-12 gap-6">
          <div className="col-span-8">
            <div className="flex items-center justify-between mb-3">
              <div className="text-[12px] font-mono uppercase tracking-[0.2em] text-white/50">tracklist · 8</div>
              <button className="text-[12px] text-white/50 hover:text-white inline-flex items-center gap-1.5">
                <IconArrow size={12}/> 순서 변경
              </button>
            </div>
            <div className="ob-glass-soft rounded-2xl overflow-hidden">
              {DEMO_TRACKS.map((t, i)=>(
                <div key={t.id} className="flex items-center px-5 py-3 border-b border-white/5 last:border-b-0 hover:bg-white/[0.03] group">
              ...
              {!submitted ? (
                <>
                  ...
                  <div className="mt-5 flex items-center gap-2">
                    {[1,2,3,4,5].map(n=>(
                      <button
                        ...
                        className="p-1"
                      >
                        <StarIcon active={(hoverRating||rating) >= n}/>
                      </button>
                    ))}
                    {rating>0 && <span className="ml-2 text-[12px] text-white/60 font-mono">{rating}/5</span>}
                  </div>
                  <textarea
                    value={feedback}
                  ...
                  <button
                    ...
                    disabled={rating===0}
                    className={`mt-3 w-full h-11 rounded-full text-[13px] font-semibold transition ${rating>0?'ob-btn-primary':'bg-white/[0.05] text-white/30 cursor-not-allowed'}`}
                  >
                    평가 보내기
                  </button>
                </>
