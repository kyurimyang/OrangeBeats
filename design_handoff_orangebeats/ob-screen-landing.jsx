// Landing screen — hero, how-it-works, featured playlists, big CTA
// Globals: LandingScreen

function LandingScreen({ onNavigate, tweaks }) {
  const [url, setUrl] = React.useState('');

  const submit = () => {
    onNavigate('url', { initialUrl: url || 'https://youtube.com/watch?v=ob-cp-mix-25' });
  };

  return (
    <div className="ob-fade pt-[68px]">
      <HeroSection url={url} setUrl={setUrl} onSubmit={submit} onNavigate={onNavigate}/>
      <HowToSection/>
      <FeaturedSection/>
      <BigCTASection onNavigate={onNavigate}/>
      <Footer/>
    </div>
  );
}

function HeroSection({ url, setUrl, onSubmit, onNavigate }) {
  return (
    <section className="relative overflow-hidden">
      ...
      <div className="mx-auto max-w-[1280px] px-8 pt-24 pb-10">
        ...
        <p className="mt-8 text-center text-white/60 text-[17px] leading-relaxed max-w-2xl mx-auto">
          Youtube Playlist를 이용하면서 힘들었던 적은 없었나요? <span className="text-white/85">orangebeats</span>가 한 번에 옮겨드려요.
          <br/>좋아하는 스트리밍에서 그대로 듣고, 수정하고, 즐기세요.
        </p>
        ...
        <div className="mt-12 max-w-[760px] mx-auto">
          ...
          <div className="mt-4 flex items-center justify-center gap-4 text-[12px] text-white/40">
            <span className="inline-flex items-center gap-1.5"><span className="w-1 h-1 rounded-full bg-mint"/> 무료 · 회원가입 필요 없음</span>
        ...
        <HeroPreview onClick={()=>onNavigate('candidates')}/>
      </div>
    </section>
  );
}

function HeroPreview({ onClick }) {
  return (
      ...
      <div className="ob-glass rounded-[28px] p-2.5 relative">
        <div className="rounded-[22px] bg-ink-900 border border-white/5 overflow-hidden">
          <div className="flex items-center justify-between px-5 py-3 border-b border-white/5">
            ...
            <div className="font-mono text-[11px] text-white/40 tracking-wider">orangebeats / candidate-matching</div>
            <div className="w-12"/>
          </div>
          <div className="grid grid-cols-12 gap-0">
            <div className="col-span-5 p-6 border-r border-white/5">
              <div className="aspect-video rounded-xl overflow-hidden ob-cover relative" style={{ '--c1':'#FD6D11', '--c2':'#2BB8A3' }}>
                ...
                <div className="absolute bottom-2 right-2 font-mono text-[10px] px-1.5 py-0.5 rounded bg-black/70">{DEMO_VIDEO.duration}</div>
              </div>
              <div className="mt-4 text-[13px] text-white/85 line-clamp-2 leading-snug">{DEMO_VIDEO.title}</div>
              <div className="mt-1 text-[12px] text-white/40">{DEMO_VIDEO.channel} · 24개 트랙 추출됨</div>
            </div>
            <div className="col-span-7 p-4">
              <div className="flex items-center justify-between px-3 py-2">
                <div className="text-[12px] text-white/50 font-mono tracking-wide">TRACKLIST · 8 of 24</div>
                ...
                {DEMO_TRACKS.slice(0,5).map(t => (
                  <div key={t.id} className="flex items-center gap-3 px-3 py-2 rounded-xl hover:bg-white/[0.04]">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-[11px] text-white/50 truncate">{t.artist} · {t.album}</div>
                    </div>
                    <ConfidencePill level={t.conf}/>
                    <div className="text-[11px] text-white/40 font-mono w-8 text-right">{t.duration}</div>
                  </div>
                ))}
                <div className="px-3 py-2 text-[11px] text-white/40 text-center font-mono">+ 19 more tracks</div>
              </div>
            </div>
          </div>
        </div>
        <button onClick={onClick} className="absolute inset-x-0 -bottom-3 mx-auto w-fit translate-y-full inline-flex items-center gap-1.5 text-[12px] text-white/50 hover:text-white">
      ...
      <div className="flex items-end justify-between mb-10">
        <div>
          ...
          <h2 className="mt-4 font-display text-[56px] tracking-[-0.03em] leading-[1.02] font-medium">
            세 번의 클릭으로,<br/><span className="ob-grad-accent">완벽한 플레이리스트.</span>
          </h2>
        </div>
        ...
        {steps.map((s)=>(
          <div key={s.n} className="ob-glass rounded-3xl p-7 relative overflow-hidden">
            <div className="flex items-start justify-between">
              <div className="w-11 h-11 rounded-2xl ob-chip flex items-center justify-center">{s.icon}</div>
            ...
            <div className="absolute -right-12 -bottom-12 w-40 h-40 rounded-full opacity-30"
        ...
        {FEATURED_PLAYLISTS.map((p, i)=>(
          <button key={p.title} className="text-left group">
            <div className="aspect-square rounded-2xl overflow-hidden relative"
                 style={{ background:`radial-gradient(120% 80% at 30% 20%, ${p.hue}33, transparent 60%), linear-gradient(160deg, ${p.hue}, #0a0a0a 90%)` }}>
              <div className="absolute inset-0 p-5 flex flex-col justify-between">
                <div className="flex items-center justify-between">
                  <span className="font-mono text-[10px] tracking-[0.2em] text-white/70">PL · {String(i+1).padStart(2,'0')}</span>
              ...
              <div className="absolute -bottom-8 -right-8 w-32 h-32 rounded-full opacity-25"
...
function BigCTASection({ onNavigate }) {
  return (
    <section className="mx-auto max-w-[1280px] px-8 mt-28">
      <div className="ob-glass rounded-[32px] p-12 relative overflow-hidden">
        <div className="grid grid-cols-12 gap-8 items-center relative z-10">
          <div className="col-span-7">
            ...
            <h2 className="mt-5 font-display text-[64px] tracking-[-0.03em] leading-[0.98] font-medium">
              <span className="ob-grad-text">지금 옮기는 데</span><br/>
              <span>약 </span><span className="text-orange">37초</span><span>면 끝나요.</span>
            </h2>
            <p className="mt-5 text-white/55 text-[15px] max-w-md leading-relaxed">
              평균 1시간짜리 플레이리스트가 37초 안에 옮겨져요. 매칭이 어려운 곡은 OCR/ACR로 한 번 더 시도해드려요.
            </p>
            <div className="mt-8 flex items-center gap-3">
              <button onClick={()=>onNavigate('url')} className="ob-btn-primary h-12 px-6 rounded-full text-[14px] font-semibold inline-flex items-center gap-2">
                지금 옮기기 <IconArrow size={14}/>
              </button>
              <button onClick={()=>onNavigate('faq')} className="ob-btn-ghost h-12 px-5 rounded-full text-[14px] text-white/80 inline-flex items-center gap-2">
                FAQ 보기
              </button>
            </div>
          </div>
        ...
        <div className="absolute -left-32 -bottom-32 w-[420px] h-[420px] rounded-full"
