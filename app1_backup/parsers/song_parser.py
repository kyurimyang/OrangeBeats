...
def parse_json_from_text(text: str) -> dict:
    """
    LLM 응답에서 JSON 블록만 추출해서 dict로 변환.
    실패하면 {"songs": []} 반환.
    """
    if not text:
        return {"songs": []}
...
def _split_pair(text: str) -> tuple[str, str] | None:
    """
    'Song - Artist' / 'Artist - Song' 같은 단순 패턴 분리
    """
    separators = [" - ", " – ", " — ", " | ", " / ", ":"]
...
def count_text_signals(text: str) -> dict:
    """
    설명란/댓글 안에 곡 목록처럼 보이는 신호가 얼마나 있는지 계산.
    """
    if not text:
        return {
            "timestamp_count": 0,
