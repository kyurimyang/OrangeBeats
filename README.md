...
> 즉, 전체 목표 중에서는 **댓글 기반 초기 추출 파이프라인까지 완료**된 상태입니다.

## 다음 우선순위 권장

1. `main.py`에 테스트 API 1개 연결 (URL -> 댓글수집 -> LLM -> JSON 반환)
2. 설명란 수집 함수 추가 후 STEP 1/2 조기 종료 로직 구현
3. Spotify 매칭 최소 버전(검색 + confidence 분류) 구현
