...
def _prune_auth_states(states: Dict[str, Any], now: Optional[int] = None) -> Dict[str, Dict[str, Any]]:
    ...
    for state, raw_value in states.items():
        ...
        redirect_uri = raw_value.get("redirect_uri")
        cleaned[state] = {
...
def save_auth_state(state: str, frontend_origin: str, redirect_uri: str) -> None:
...
def save_token_data(token_data: Dict[str, Any]) -> None:
    ...
    try:
        ...
        print("spotify session save failed =", str(exc))
...
def clear_tokens() -> None:
    ...
    try:
        ...
        print("spotify session clear failed =", str(exc))
